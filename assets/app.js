...
function exportMultiPageZip() {
    ...
    try {
        const pageTypes = ['homepage', 'about', 'services', 'portfolio', 'contact', 'blog'];
        
        pageTypes.forEach(pageType => {
            // Generate content for each page type
            const pageContent = generatePageContent(pageType, data, colors, fonts);
            const layout = DesignTemplates.layouts[data.layout];
            
            if (layout && pageContent) {
                const htmlContent = layout.generate(pageContent, data.style, colors);
                const completeHTML = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${pageContent.siteTitle} - ${pageContent.pageTitle}</title>
    <style>
...
${htmlContent}
                ...
                zip.file(`${pageType}.html`, completeHTML);
            }
        });
        
        // Add shared CSS file
        const sharedCSS = generateSharedCSS(colors, fonts);
        zip.file('styles.css', sharedCSS);
        
        // Add README
        const readmeContent = `# ${data.siteTitle} - Multi-Page Website

This package contains a complete multi-page website with the following pages:
...
4. All pages share consistent design and branding
...
${new Date().toLocaleDateString()}`;
        ...
        zip.generateAsync({ type: 'blob' })
            .then(function(content) {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = `${data.siteTitle.toLowerCase().replace(/\s+/g, '-')}-multipage-website.zip`;
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
                URL.revokeObjectURL(url);
            ...
            .catch(function(error) {
                console.error('Multi-page ZIP generation failed:', error);
                alert('Failed to create multi-page ZIP. Please try again.');
            });
            
    } catch (error) {
        console.error('Multi-page export error:', error);
        alert('Failed to create multi-page export. Please try again.');
    }
}

function generatePageContent(pageType, baseData, colors, fonts) {
    const pageConfigs = {
        homepage: {
            pageTitle: 'Home',
    ...
    return pageConfigs[pageType] || pageConfigs.homepage;
}

function generateSharedCSS(colors, fonts) {
    return `
...
function createAndDownloadZip(exportData, filename) {
    ...
    try {
        // Add main file
        if (exportData.main) {
            zip.file(exportData.main.filename, exportData.main.content);
        }
        
        // Add CSS file
        if (exportData.styles) {
            zip.file(exportData.styles.filename, exportData.styles.content);
        }
        
        // Add design tokens
        if (exportData.tokens) {
            zip.file(exportData.tokens.filename, exportData.tokens.content);
        }
        
        // Add additional files if they exist
        if (exportData.additional) {
            exportData.additional.forEach(file => {
...
This is your exported design template package.

## Contents:
${exportData.main ? `- ${exportData.main.filename} - Main template file\n` : ''}${exportData.styles ? `- ${exportData.styles.filename} - Styles and CSS\n` : ''}${exportData.tokens ? `- ${exportData.tokens.filename} - Design tokens\n` : ''}
## Usage:
